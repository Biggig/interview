...
高级、中级和低级调度作业从提交开始直到完成，往往要经历下述三级调度：
  ...
  - 高级调度：
  1. 又称为作业调度，它决定把后备作业调入内存运行；  
  2. 内存和辅存之间的调度，对于每个作业只调入一次，调出一次；  
   
  - 中级调度：
  1. 又称为在虚拟存储器中引入，在内、外存对换区进行进程对换；  
  2. 将暂时不能运行的进程调至外存等待，挂起态；  
  3. 将外存中已具备运行条件的就绪进程重新调入内存，修改为就绪态  
...
- `破坏互斥条件`：允许系统资源都能共享使用；SPOOLing技术  
  
- `破坏不剥夺条件`：当一个已保持了某些不可剥夺资源的进程请求新的资源而得不到满足时，必须释放已保持的所有资源；或由操作系统协助，剥夺别的进程的资源  

- `破坏请求并保持条件`：运行前一次申请所需要的全部资源，在资源未满足前，不投入运行  
  
- `破环循环等待条件`：顺序资源分配法，给系统中的资源编号，规定每个进程必须按照编号递增顺序请求资源

#### 死锁避免

1. **互斥锁的加锁规则**：给定所有互斥操作一个全序，如果每个线程均按这种顺序获得互斥锁，并且按相反顺序解锁，则不会发生死锁。  
2. **安全性算法**：银行家算法

#### 死锁检测和解除

1. **资源分配图**
2. **死锁解除**： `资源剥夺法`， `撤销进程法`，`进程回退法` 
